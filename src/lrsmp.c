...
bigu:
  if (zero (v))
    return;
  if ((i = mp_length (u)) < maxsplen || (i == maxsplen && u[maxsplen - 1] < maxspval))
    goto quickfinish;
  divint (u, v, r);
  normalize (u);
...
bigv:
  ...
  if ((i = mp_length (v)) < maxsplen || (i == maxsplen && v[maxsplen - 1] < maxspval))
    goto quickfinish;
  divint (v, u, r);
  normalize (v);
  goto bigu;
 ...
 quickfinish:
 ul = vl = 0;
  for (i = mp_length (u) - 1; i > 0; i--)
    ul = BASE * ul + u[i];
  for (i = mp_length (v) - 1; i > 0; i--)
    vl = BASE * vl + v[i];
  if (ul > vl)
    goto qv;
qu:
...
string prat (char name[], lrs_mp Nin, lrs_mp Din)	/*reduce and print Nin/Din  */
{
	...
	ss<<Nt[mp_length(Nt) -1];

	for (i = mp_length (Nt) - 2; i >= 1; i--)
		ss<<Nt[i];
	if (!(Dt[0] == 2 && Dt[1] == 1)){
		/* rational */
		ss<<"/";
		ss<<Dt[mp_length(Dt) -1];
		for (i = mp_length (Dt) - 2; i >= 1; i--)
			ss<<Dt[i];
	}
	ss<<" ";
...
void prat (char name[], lrs_mp Nin, lrs_mp Din)	/*reduce and print Nin/Din  */
{
    ...
    if (sign (Nin) * sign (Din) == NEG)
      ...
      fprintf (lrs_ofp, " ");
    fprintf (lrs_ofp, "%lu", Nt[mp_length (Nt) - 1]);
    for (i = mp_length (Nt) - 2; i >= 1; i--)
      fprintf (lrs_ofp, FORMAT, Nt[i]);
    if (!(Dt[0] == 2 && Dt[1] == 1))	/* rational */
      { 
	fprintf (lrs_ofp, "/");
	fprintf (lrs_ofp, "%lu", Dt[mp_length (Dt) - 1]);
	for (i = mp_length (Dt) - 2; i >= 1; i--)
...
void pmp (char name[], lrs_mp a)	/*print the long precision integer a */
{
  ...
  if (sign (a) ==NEG)
    ...
    fprintf (lrs_ofp, " "); 
  fprintf (lrs_ofp, "%lu", a[mp_length (a) - 1]); 
  for (i = mp_length (a) - 2; i >= 1; i--)
